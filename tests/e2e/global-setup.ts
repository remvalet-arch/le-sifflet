...
const TEST_USER_EMAIL   = process.env.TEST_USER_EMAIL   ?? "e2e-player@le-sifflet.test";
const TEST_USER_PASSWORD = process.env.TEST_USER_PASSWORD ?? "S1fflet-E2E-2026!";
...
async function ensureTestUser() {
  ...
  if (existing) {
    console.log(`[setup] Utilisateur E2E existant : ${existing.id.slice(0, 8)}…`);
    return;
  }
  ...
  if (error) throw new Error(`Impossible de créer l'utilisateur E2E : ${error.message}`);
...
export default async function globalSetup(_config: FullConfig) {
  if (!TEST_AUTH_SECRET) {
    throw new Error("TEST_AUTH_SECRET manquant dans .env.local — requis pour les tests E2E.");
  }
