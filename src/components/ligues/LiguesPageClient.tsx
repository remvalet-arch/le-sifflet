...
import { Copy, Check, Users, Swords, Plus, Hash, LoaderCircle, Trophy, Wallet } from "lucide-react";
...
type SquadWithMembers = SquadRow & { members: SquadMember[]; pot_commun: number };
    ...
    <div className="space-y-5">
      ...
      {tab === "creer" && (
        <div className="space-y-4">
          ...
          >
            <span className="text-sm font-bold text-zinc-300">Ligue privée (code d&apos;invitation)</span>
            <span
      ...
      {tab === "mes" && (
        <>
          <p className="text-sm text-zinc-500">
            ...
            <span className="font-bold text-amber-400/90">braquage</span> sur les paris en direct. Le{" "}
            <span className="font-bold text-white">pot commun</span>, c&apos;est la somme des Pts des membres (solde
            actuel).
          </p>
          ...
          {squads === null ? (
            ...
            <ul className="flex flex-col gap-3">
              {squads.map((s) => (
                <li key={s.id} className="rounded-2xl border border-white/10 bg-zinc-900/70 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-2 flex items-center gap-2 text-sm font-bold text-green-400/90">
                        <Wallet className="h-4 w-4 shrink-0" aria-hidden />
                        Pot commun :{" "}
                        <span className="font-black tabular-nums">{s.pot_commun.toLocaleString("fr-FR")} Pts</span>
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <Link
                        href={`/ligues/${s.id}`}
                        className="inline-flex items-center gap-1 rounded-xl border border-zinc-600 px-3 py-2 text-xs font-black uppercase tracking-wide text-zinc-200 hover:border-amber-500/50 hover:text-amber-300"
                      >
            ...
            <p className="rounded-2xl border border-dashed border-zinc-700 py-8 text-center text-sm text-zinc-500">
              Aucune ligue pour l&apos;instant — crée-en une ou rejoins avec un code.
            </p>
          )}
