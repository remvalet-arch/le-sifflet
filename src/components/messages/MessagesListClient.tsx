    ...
    <main className="mx-auto w-full max-w-md flex-1 px-4 py-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        {friendsWithoutThread.length > 0 && (
          <details className="relative">
            <summary className="flex size-9 cursor-pointer list-none items-center justify-center rounded-xl border border-white/10 bg-white/5 text-zinc-400 transition hover:text-white active:scale-95">
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-16 text-center">
          <MessageCircle className="size-10 text-zinc-600" aria-hidden="true" />
        ...
        <ul className="flex flex-col gap-2">
          {filtered.map(({ thread, otherId, other, hasUnread }) => (
            <li key={thread.id}>
              ...
              >
                <div className="relative flex size-10 shrink-0 items-center justify-center rounded-full bg-zinc-700 text-base font-black text-zinc-300">
                  {other?.avatar_url ? (
                    <Image
                  ...
                  {hasUnread && (
                    <span className="absolute -right-0.5 -top-0.5 size-3 rounded-full bg-whistle ring-2 ring-zinc-950" />
                  )}
                </div>
