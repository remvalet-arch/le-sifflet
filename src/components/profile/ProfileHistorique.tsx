...
function fmtDate(iso: string, bcp47: string) {
...
function fmtStartDate(iso: string, bcp47: string) {
...
function PronoRow({ prono: p }: { prono: PronoEntry }) {
  ...
  function formatLine(): string {
    const v = p.prono_value;
    if (v === "🔒") return t("pronoHidden");
    if (p.prono_type === "exact_score")
      return t("pronoExactScore", { score: v });
    if (p.prono_type === "scorer") return t("pronoScorer", { name: v });
    if (p.prono_type === "scorer_allocation") {
      try {
        ...
        if (parsed.home) parsed.home.forEach((s) => names.push(fmtScorer(s)));
        if (parsed.away) parsed.away.forEach((s) => names.push(fmtScorer(s)));
        if (names.length === 0)
          return t("pronoScorers", { names: t("pronoNoScorer") });
        return t("pronoScorers", { names: names.join(", ") });
      } catch {
        return t("pronoScorers", { names: t("pronoError") });
      }
    }
    return v;
  }
...
function VarBetRow({ bet: b }: { bet: ShortBetEntry }) {
  ...
  const eventLabel = b.eventType
    ? ({
        penalty_check: { label: t("eventPenaltyCheck"), emoji: "📢" },
        penalty_outcome: { label: t("eventPenaltyOutcome"), emoji: "🥅" },
        var_goal: { label: t("eventVarGoal"), emoji: "🚩" },
        red_card: { label: t("eventRedCard"), emoji: "🟥" },
        free_kick: { label: t("eventFreekick"), emoji: "🎯" },
        corner: { label: t("eventCorner"), emoji: "🏁" },
        stoppage_ht: { label: t("eventStoppageHt"), emoji: "⏱️" },
        stoppage_ft: { label: t("eventStoppageFt"), emoji: "⏱️" },
      }[b.eventType] ?? { label: b.eventType, emoji: "⚡" })
    : { label: "VAR", emoji: "⚡" };
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-[10px] text-zinc-600">
          ...
          {" · "}
          {t("staked", { amount: b.amount_staked })}
        </p>
      </div>
      <div className="shrink-0 text-right">
        <span
    ...
    <div className="flex flex-col gap-3">
      {recentEntries.length > 0 && (
        ...
        >
          ...
          >
            {recentPts > 0 ? "+" : ""}
            {t("sevenDaysPoints", { pts: recentPts.toLocaleString(bcp47) })}
            {recentWon > 0 && (
              <span className="ml-2 font-semibold text-zinc-400">
                {t("sevenDaysWon", { count: recentWon })}
              </span>
            )}
            {recentLost > 0 && (
              <span className="ml-1 font-semibold text-zinc-400">
                {t("sevenDaysLost", { count: recentLost })}
              </span>
            )}
            {recentPending > 0 && (
              <span className="ml-1 font-semibold text-zinc-400">
                {t("sevenDaysPending", { count: recentPending })}
              </span>
            )}
          </p>
        </div>
              ...
              <div className="shrink-0">
                {g.matchStatus === "live" ? (
                  ...
                  <span className="rounded-full border border-zinc-700 bg-zinc-900 px-2 py-0.5 text-[9px] font-semibold text-zinc-500">
                    {fmtStartDate(g.startTime, bcp47)}
                  </span>
                ) : null}
              </div>
            </div>
