...
export default async function LobbyPage() {
  ...
  const [matchesResult, profileResult] = await Promise.all([
  ...
  if (matchesResult.error) {
    return (
      <div className="px-4 py-8 text-sm text-red-400">
        Impossible de charger les matchs ({matchesResult.error.message}).
      </div>
    );
  }
    ...
    <>
      <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-6">
        {matches.length === 0 ? (
          ...
          <>
            {live.length > 0 && (
              <Section title="En Direct" live matches={live} />
            )}
            {upcoming.length > 0 && (
              <Section
                title="À venir"
