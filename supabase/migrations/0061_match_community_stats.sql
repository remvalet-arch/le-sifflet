-- 0061_match_community_stats.sql
...
WITH score_counts AS (
  SELECT
    ...
    count(*) FILTER (WHERE split_part(prono_value, '-', 1)::int > split_part(prono_value, '-', 2)::int) as votes_1,
    count(*) FILTER (WHERE split_part(prono_value, '-', 1)::int = split_part(prono_value, '-', 2)::int) as votes_N,
    count(*) FILTER (WHERE split_part(prono_value, '-', 1)::int < split_part(prono_value, '-', 2)::int) as votes_2
  FROM pronos
...
SELECT
  ...
  CASE WHEN sc.total_pronos > 0 THEN round((sc.votes_1::numeric / sc.total_pronos) * 100) ELSE 0 END as community_1_pct,
  CASE WHEN sc.total_pronos > 0 THEN round((sc.votes_N::numeric / sc.total_pronos) * 100) ELSE 0 END as community_N_pct,
  CASE WHEN sc.total_pronos > 0 THEN round((sc.votes_2::numeric / sc.total_pronos) * 100) ELSE 0 END as community_2_pct,
