-- 0059 : Moteur de Points "Contre-Pied" — points_earned + contre_pied_bonus sur pronos
-- Formule : 50 pts base (bonne issue 1N2) + Prime Contre-Pied si score exact
--   > 40% des bons 1N2 avaient le score exact → +10 pts (Le Minimum Syndical)
--   15-40%                                    → +30 pts (Le Joli Coup)
--   5-15%                                     → +60 pts (Le Visionnaire)
--   < 5%                                      → +100 pts (Le Braquage)
--   Fallback si < 5 joueurs avec bon 1N2      → +30 pts

-- 1. Nouvelles colonnes
ALTER TABLE public.pronos
  ADD COLUMN IF NOT EXISTS points_earned integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS contre_pied_bonus integer NOT NULL DEFAULT 0;

-- 2. Moteur de résolution mis à jour
...
COMMENT ON FUNCTION public.resolve_match_pronos(uuid) IS
  'Résout les pronos pending : 50 pts base (bonne issue 1N2) + prime Contre-Pied si score exact ; idempotent.';
