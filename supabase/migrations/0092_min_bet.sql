-- Sprint Eco-4 : Mises minimum scalantes
-- Étend place_bet pour rejeter les mises sous le seuil dynamique du solde

-- Fonction utilitaire : min-bet selon le solde
CREATE OR REPLACE FUNCTION public.get_min_bet_for_balance(p_balance INT)
RETURNS INT
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_balance < 5000   THEN 5
    WHEN p_balance < 20000  THEN 50
    WHEN p_balance < 50000  THEN 200
    WHEN p_balance < 100000 THEN 500
    ELSE 1000
  END;
$$;

-- Extension de place_bet : remplace le check fixe à 10 par le seuil dynamique
